...
double speed_mode_set = 0.0;
double turret_mode_set = 0.0;
 ...
 * this 2 seconds is handled by spa_wn asteroid that flush LED at 2Hz which is equivalent to 2 seconds
...
void turretmode_off(){
    if (elapsed - turret_mode_set > 1.0){
        turretMode = false;
    }
}
...
void after_game_over(void){
    for (int l = 50; l < 1024; l+=20){
        clear_screen();
        draw_string(0, 10, "Quit:right btn", FG_COLOUR);
        draw_string(0, 20, "Reset:left btn", FG_COLOUR);
        show_screen();
        set_duty_cycle(l);
        _delay_ms(30);
    }
    ...
    while(1){
        if(BIT_IS_SET(PINF, 5) ||
           (char)usb_serial_getchar() == 'q'){
            do_quit();
            break;
        } else if (BIT_IS_SET(PINF, 6) ||
                   (char)usb_serial_getchar() == 'r'){
            game_over = false;
            do_reset();
            break;
        }
...
void further_input(char ch){
    ...
    if (input_mode == 'n' && isNumber(ch)){
        ...
        if (input_command == 'o' && input_turret_minus){
 ...
 */
bool no_objects_on_screen(){
    return (current_asteroid == 0 && current_boulder == 0 && current_fragment == 0 && !paused);
}
